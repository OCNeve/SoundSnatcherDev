...
def defaultUnixDownloadPath():
    return osjoin(expanduser('~'), 'downloads')
...
def get_path(folderid, user_handle=None):
    if osname == 'nt':
        if user_handle == None:
            user_handle = UserHandle.common
        fid = GUID(getattr(FOLDERID,folderid))
        ...
        return defaultUnixDownloadPath()
